...
double sdf_mandelbulb(const double vec_p[NR_VEC_ELEMENTS], double params[])
{
	...
	for(int i = 0; i < 4; i++)
    {
        ...
        double b = (a == 0) ? 0.0 : 1.0/sqrt(a*a2*a4);
        double c = x4 + y4 + z4 - 6.0*y2*z2 - 6.0*x2*y2 + 2.0*z2*x2;
        double d = x2 - y2 + z2;

        x = vec_p[0] +  64.0*x*y*z*(x2-z2)*d*(x4-6.0*x2*z2+z4)*c*b;

        // z = vec_p[2] +  -8.0*y*d*(x4*x4 - 28.0*x4*x2*z2 + 70.0*x4*z4 - 28.0*x2*z2*z4 + z4*z4)*c*b;
        z = vec_p[2] +  8.0*y*d*( 28.0*x2*z2*(x4 + z4)  - 70.0*x4*z4  - z4*z4 - x4*x4 )*c*b;
