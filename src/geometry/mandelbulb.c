...
double sdf_mandelbulb(const double vec_p[NR_VEC_ELEMENTS], double params[])
{
	...
	for(int i = 0; i < 4; i++)
    {
        ...
        double k3 = x2 + z2;
        double k3_2 = k3*k3;
        double k3_4 = k3_2*k3_2;
        double k2 = (k3 == 0) ? 0.0 : 1.0/sqrt(k3*k3_2*k3_4);
        double k1 = x4 + y4 + z4 - 6.0*y2*z2 - 6.0*x2*y2 + 2.0*z2*x2;
        double k4 = x2 - y2 + z2;

        x = vec_p[0] +  64.0*x*y*z*(x2-z2)*k4*(x4-6.0*x2*z2+z4)*k1*k2;
        z = vec_p[2] +  -8.0*y*k4*(x4*x4 - 28.0*x4*x2*z2 + 70.0*x4*z4 - 28.0*x2*z2*z4 + z4*z4)*k1*k2;
        y = vec_p[1] + -16.0*y2*k3*k4*k4 + k1*k1;
