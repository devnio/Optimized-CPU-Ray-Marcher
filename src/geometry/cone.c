...
{
    Vec3 q;
    new_vector(&q, sqrt(p.x * p.x + p.z * p.z), p.y, 0.0);
    Vec3 k1;
    new_vector(&k1, params[1], params[2], 0.);
    Vec3 k2; 
    new_vector(&k2, params[1] - params[0], 2. * params[2], 0);
    Vec3 ca;
    new_vector(&ca, q.x - fmin(q.x, (q.y < 0.) ? params[0] : params[1]), fabs(q.y) - params[2], 0.);
    Vec3 tmp, v_sub;
    vec_sub(&k1, &q, &v_sub);
