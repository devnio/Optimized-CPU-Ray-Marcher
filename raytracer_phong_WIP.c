...
Vector3 trace(Vector3 o, Vector3 dir, Sphere sps[], int depth)
{
    ...
    if ((depth < MAX_RAY_DEPTH) && (nearestSp->refl > 0)) 
    {
        ...
        Vector3 reflectionCol = trace(vec_add(phit, vec_mult1(N, bias)), reflDir, sps, depth + 1);
        ...
        finalColor = vec_mult1(reflectionCol, nearestSp->refl);
        bounces++;
    }
    ...
    Vector3 specularColor = new_vector(255, 255, 255);
...
void render(Sphere sps[])
{
    ...
    for (unsigned y = 0; y < height; ++y)
    {
        for (unsigned x = 0; x < width; ++x)
        {
            ...
            px_col = vec_mult1(px_col, multiplier/bounces);  // TODO: should divide by a counter of successful bounces instead
            bounces = 1;
...
int main()
{
    ...
    Sphere sp1; sp1.c = new_vector(4, 0, -25); sp1.r = 3; sp1.r2 = 9; sp1.surfCol = new_vector(1, 0.32, 0.36); sp1.refl=0.5; sp1.emissionColor = new_vector(0,0,0);
    ...
    Sphere sp3; sp3.c = new_vector(0, 0, -40); sp3.r = 3; sp3.r2 = 9; sp3.surfCol = new_vector(0.65, 0.77, 0.97); sp3.refl=0; sp3.emissionColor = new_vector(0,0,0);
